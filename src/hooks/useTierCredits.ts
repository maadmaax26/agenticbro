...
export function useTierCredits(walletAddress: string | null): TierCreditsState {
  ...
  const useCredit = useCallback((): { success: boolean; remaining: number; type: 'tier' | 'paid' } => {
    ...
    if (state.tierScansRemaining > 0) {
      const newUsed = state.tierScansUsed + 1;
      ...
      return { success: true, remaining: state.tierMonthlyScans - newUsed, type: 'tier' };
    }

    // Use paid credits
    if (state.paidCredits > 0) {
      const newCredits = state.paidCredits - 1;
  ...
  const addCredits = useCallback((amount: number) => {
    ...
    const newCredits = state.paidCredits + amount;
    localStorage.setItem(`agenticbro_credits_${walletAddress.toLowerCase()}`, String(newCredits));
