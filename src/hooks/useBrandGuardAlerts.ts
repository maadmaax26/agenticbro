...
export function useBrandGuardAlerts(userId: string | null) {
  ...
  // Alerts are generated by the alert-delivery cron (every 15 min) and monitor-worker
  // (every 6 h), so 5-minute polling is more than sufficient and cuts DB calls 10×.
  // Visibility-aware: pauses when the tab is hidden, refreshes immediately on focus.
  useEffect(() => {
    fetchAlerts();
    if (!userId) return;

    const POLL_MS = 5 * 60_000; // 5 minutes
    let interval = setInterval(fetchAlerts, POLL_MS);

    const handleVisibility = () => {
      if (document.visibilityState === 'visible') {
        fetchAlerts(); // catch up after tab was backgrounded
        clearInterval(interval);
        interval = setInterval(fetchAlerts, POLL_MS);
      } else {
        clearInterval(interval);
      }
    };
