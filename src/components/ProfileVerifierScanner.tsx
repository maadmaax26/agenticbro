  ...
  for (const instance of nitterInstances) {
    try {
      ...
      return {
        ...
        riskScore: 0,
        riskLevel: 'LOW',
        redFlags: ['Public web fallback retrieved profile metadata only'],
        evidence: [],
        recommendation: 'Risk scoring is performed by the private intelligence service.',
        profileData: {
          followers,
          following,
          bio,
...
function generateDemoResult(platform: string, username: string): ProfileScanResult {
  ...
  return {
    ...
    riskScore: 0,
    riskLevel: 'LOW',
    redFlags: ['Public demo fallback does not perform risk scoring'],
    evidence: ['Private intelligence service required for profile risk scoring'],
    recommendation: 'Risk scoring is performed by the private intelligence service.',
    profileData: {
      bio: `${username} - public fallback`,
      joinDate: new Date().toISOString().split('T')[0],
