...
export async function generateTakedownReport(
  ...
  if (input.user?.id) {
    insertData.user_id = input.user.id;
  }
  if (input.brand?.trademarkNumber) {
    insertData.brand_id = input.brand.trademarkNumber; // or use a proper brand_id
  }
  ...
  const takedownAction: any = {
    brand_monitor_id: input.scanId, // link to the scan/brand
    platform: input.platform,
    action_type: 'report',
    status: 'pending',
  ...
  if (input.user?.id) {
    takedownAction.user_id = input.user.id;
  }

  // Insert into takedown_actions so it shows in the Takedown Center
  try {
    const { error: taError } = await supabase.from('takedown_actions').insert(takedownAction);
    if (taError) console.error('Failed to insert takedown_action:', taError.message);
  } catch (e) {
    console.error('Failed to insert takedown_action:', (e as Error).message);
  }
