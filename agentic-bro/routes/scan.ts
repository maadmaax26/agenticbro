...
router.post('/profile', async (req: Request, res: Response) => {
  try {
    ...
    // ── Auto-chain URL scan if profile has a website ────────────────────────
    // When a profile contains a website link, auto-submit a JS detonation
    // scan and include the result in the profile report.
    ...
    if (websiteUrl && typeof websiteUrl === 'string' && websiteUrl.startsWith('http')) {
      try {
        ...
        const sbKey = process.env.SUPABASE_SECRET_API_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
        if (sbUrl && sbKey) {
        ...
        // Non-critical — profile scan still returns without URL scan
        console.error('[profile-scan] URL chain error:', chainErr);
      }
    }
