...
export class RateLimiter {
   ...
   */
  scanLimit = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const limits = TIER_LIMITS[tier as keyof TierLimits] || TIER_LIMITS.free;
   ...
   */
  verifyLimit = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const user = (req as any).user;
      if (!user) {
        return res.status(401).json({
   ...
   */
  trackUsage = async (userId: string, endpoint: string): Promise<void> => {
    try {
   ...
   */
  getUsageStats = async (userId: string, days: number = 30): Promise<UsageStats> => {
...
interface UsageStats {
  ...
  byDay: Record<string, { scans: number; verifications: number }>;
}
