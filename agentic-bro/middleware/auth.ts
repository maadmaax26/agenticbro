...
export class Auth {
   ...
   */
  optionalApiKey = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const apiKey = this.extractApiKey(req);
   ...
   */
  private async resetDailyScans(userId: string): Promise<void> {
    const query = `
      UPDATE users 
   ...
   * Create new user
   */
  async createUser(data: { telegramId?: number; email?: string; tier?: string }): Promise<User> {
    const query = `
