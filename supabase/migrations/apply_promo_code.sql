...
-- 2. Update transaction type constraint
...
ALTER TABLE brand_guard_credit_transactions ADD CONSTRAINT brand_guard_credit_transactions_transaction_type_check
  CHECK (transaction_type IN (
    'free_grant', 'free_usage', 'purchase', 'purchase_bonus', 'paid_usage',
    'subscription_grant', 'refund', 'admin_adjustment', 'promo_grant'
  ));

-- 3. Update initialize function (must run as one block)
