/**
 * Public Brand Guard domain monitor shell.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const result = await callPrivateIntel('/v1/brand-guard/domain-monitor', {
      method: 'POST',
      body,
    ...
    sendJson(res, statusCode, {
      success: false,
      error: error instanceof Error ? error.message : 'Private domain monitoring failed',
    });
  }
}
