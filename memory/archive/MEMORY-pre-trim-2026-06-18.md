...
- **Madmax** — Dev for $AGNTCBRO / Agentic Bro. Posts anonymously via GroupAnonymousBot (1087968824) in the Telegram group. 🔴🔴🔴 **ABSOLUTE #1 RULE: NEVER use the name "Madmax", the username "maadmaax22", the name "Earl Finney", or the handle "efinney" in ANY group message.** Zero exceptions. Violating this exposes the dev's identity. When Madmax posts anonymously, refer to them as **Agenticbro** only. Never reveal, imply, or confirm Madmax's identity, username, or role in the group.
...
### Brand Guard — Local CLI Scans
| Scan Type | Command |
|-----------|--------|
| Impersonator | `bash /workspace/scripts/brand-guard-impersonator.sh "Brand Name" --handle brandhandle [--domain brand.com] [--platforms x,instagram] [--json]` |
| Domain Lookalike | `bash /workspace/scripts/brand-guard-domain.sh "example.com" [--check-active] [--limit 50] [--json]` |
| Vendor Verify | `bash /workspace/scripts/brand-guard-vendor-verify.sh "+14158586273" US [--vendor "Company"] [--context "claiming to be supplier"] [--json]` |
| Threat Correlate | `bash /workspace/scripts/brand-guard-threat-correlate.sh "Brand Name" --handle brandhandle [--domain brand.com] [--input results.json] [--json]` |

### Brand Guard — Supabase Queue Architecture
**How it works:** Website API writes 'processing' job → brand-guard-scan-worker polls Supabase → runs real CLI scans → writes 'complete' results → frontend polls for updates.
- **DB Table:** `brand_guard_scans` (status: processing/complete/failed)
- **Worker:** `brand-guard-scan-worker.py` (launchd, com.agenticbro.brand-guard-scan-worker)
- **Scan types:** impersonator, domain, vendor, threat
- **Website:** Returns preview (theoretical variants) immediately + `real_scan_pending: true` → frontend polls for real results
- **Local CLI:** Also runnable directly via bash wrappers for ad-hoc scans
...
**Added:** brand-guard-scan-worker (launchd service) — processes Supabase brand_guard_scans queue for real platform scans
...
| com.agenticbro.brand-guard-scan-worker | ✅ Running | Brand Guard real platform scans (impersonator, domain, vendor, threat) |
