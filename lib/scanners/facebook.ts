 ...
 * ========================
 * Fetches Facebook profile HTML and runs unified scoring.
 * Identical logic to Python scan-facebook.py
 */
...
export async function scanFacebook(username: string): Promise<FacebookScanResult> {
  ...
  try {
    ...
    if (content.includes('This page isn\'t available') || content.includes('Page Not Found')) {
      return {
        url,
    ...
    const result = calculateRiskScore(content, 'facebook', metadata);
    ...
    if (err?.name === 'TimeoutError' || err?.name === 'AbortError') {
      return {
        url,
        error: 'Request timeout — Facebook blocking direct access',
        riskScore: 0,
        riskLevel: 'ERROR',
        verificationLevel: 'ERROR',
        redFlagsDetected: 0,
